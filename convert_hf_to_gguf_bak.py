...
@ModelBase.register(
    "LLaMAForCausalLM",
    "LlamaForCausalLM",
    "LlamaCMoEForCausalLM",  # CMoE (Carved Mixture-of-Experts) support
    "MistralForCausalLM",
    "MixtralForCausalLM",
    "VLlama3ForCausalLM",
...
class LlamaModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        # Note: CMoE models are converted to Dense format for llama.cpp compatibility
        # MoE structure is merged back by concatenating experts along intermediate_size dimension
        # This means the output GGUF file is a standard Llama model, not MoE
        if self.hf_arch == "LlamaCMoEForCausalLM":
            moe_config = hparams.get('moe_config', {})
            logger.info(f"gguf: CMoE model detected (experts={moe_config.get('num_experts', 8)}), "
                       f"merging to Dense format for llama.cpp compatibility")
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # Handle CMoE (Carved Mixture-of-Experts) model weights
        # Option 1: Convert to GGUF MoE format (requires llama.cpp MoE support)
        # Option 2: Merge back to Dense format (better compatibility)
        # Currently using Option 2 for better compatibility with llama.cpp
        if self.hf_arch == "LlamaCMoEForCausalLM":
            # Skip router weights (gate.gate, gate.classifier, gate.extra_scale, gate.extra_bias)
            if ".mlp.gate." in name:
                return []
            
            # Merge CMoE experts back to Dense format for llama.cpp compatibility
            # This loses MoE structure but ensures compatibility with standard llama.cpp
            if ".mlp.experts." in name or ".mlp.shared_experts." in name:
                if not hasattr(self, '_cmoe_experts'):
                    self._cmoe_experts = [{} for _ in range(self.block_count)]
                ...
                if match:
                    ...
                    self._cmoe_experts[layer_idx][name] = data_torch
                    
                    # Get moe_config
                    moe_config = self.hparams.get('moe_config', {})
                    num_experts = moe_config.get('num_experts', 8)
                    num_shared = moe_config.get('num_shared_experts', 1)
                    num_routed = num_experts - num_shared
                    
                    # We need 3 weights per expert (gate_proj, up_proj, down_proj)
                    # Total: (num_routed * 3) + (num_shared * 3) = num_experts * 3
                    expected_count = num_experts * 3
                    current_count = len(self._cmoe_experts[layer_idx])
                    
                    if current_count >= expected_count:
                        # Merge all experts back to standard Dense MLP format
                        # Concatenate all experts along the intermediate dimension
                        tensors = []
                        for weight_type in ['gate_proj', 'up_proj', 'down_proj']:
                            expert_weights = []
                            
                            # Collect routed experts first
                            for exp_idx in range(num_routed):
                                exp_name = f"model.layers.{layer_idx}.mlp.experts.{exp_idx}.{weight_type}.weight"
                                if exp_name in self._cmoe_experts[layer_idx]:
                                    expert_weights.append(self._cmoe_experts[layer_idx][exp_name])
                            
                            # Collect shared experts
                            shared_name = f"model.layers.{layer_idx}.mlp.shared_experts.{weight_type}.weight"
                            if shared_name in self._cmoe_experts[layer_idx]:
                                shared_weight = self._cmoe_experts[layer_idx][shared_name]
                                expert_weights.append(shared_weight)
                            
                            if expert_weights:
                                # Concatenate along the first dimension (intermediate_size)
                                # This restores the original intermediate_size (e.g., 11008 for Llama-2-7b)
                                merged_weight = torch.cat(expert_weights, dim=0)
                                
                                # Map to standard MLP format
                                standard_name = f"model.layers.{layer_idx}.mlp.{weight_type}.weight"
                                new_name = self.map_tensor_name(standard_name)
                                tensors.append((new_name, merged_weight))
                        
                        # Clear this layer's experts cache
                        self._cmoe_experts[layer_idx] = {}
                        return tensors
                    else:
                        # Not all experts loaded yet, skip for now
                        return []
                else:
                    return []
            
            # Skip router weights and any other MoE-specific weights
            if ".mlp.gate." in name or (".mlp." in name and "experts" in name):
                return []
